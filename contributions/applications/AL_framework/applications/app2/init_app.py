...
def train_initial_model():
    ...
    # Update model status here also - when run from cmd line
    app_json = get_config_for_app()
    new_model_status = 1
    app_json['model_status'] = new_model_status
    write_app_config(app_json)
...
def train(args):
    ...
    # TODO: ensure module loading works on all systems
    ...
    # TODO: trigger test script to run and patch selection to begin
    start_test_and_patch_selection()
...
def start_test_and_patch_selection():
    # module_name = 'contributions.applications.AL_framework.applications.app' + str(app_id) + '.test'
    module_name = 'test'
    mod = import_module(module_name)
    met = vars(mod)['test_app_func']
    met()
